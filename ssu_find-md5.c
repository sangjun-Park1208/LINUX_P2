...
int COUNT_MD5; // DEBUG : number of function call : sha1()
...
void md5(FILE* f, unsigned char* hash); // get hash value
off_t get_fileSize(char* path);
int check_ext(char* Ext, char* tmp_path);
int check_size(char* Min, char* Max, char* tmp_path);
int MD5_Init(MD5_CTX* c); // md5
int MD5_Update(MD5_CTX* c, const void* data, unsigned long len); // md5
int MD5_Final(unsigned char* md, MD5_CTX* c); // md5
...
void deleteNode_trash(Queue* dupSet, int SET_IDX, int REC_IDX, int k){ // [t] OPTION
	int _uid = getuid();
	if(getuid() == 0){ // if usr is root, to get username (ex : OSLAB), set user id -> 1000
		if(setuid(1000) < 0){
